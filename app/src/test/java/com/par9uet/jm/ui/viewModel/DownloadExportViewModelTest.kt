...
class DownloadExportViewModelTest {
    ...
    private class Operations : DownloadExportOperations {
        ...
        var inspection: suspend (List<DownloadComic>) -> DownloadCacheSummary = { DownloadCacheSummary(it.size, 10) }
        override suspend fun inspect(chapters: List<DownloadComic>, cachePath: String) = inspection(chapters)
    ...
    @Test fun `picker uses confirmed chapter and mode snapshot and duplicate callbacks cannot export twice`() = runTest(scheduler) {
        ...
        assertTrue(vm.prepareExport(listOf(downloadTask(1), downloadTask(2)), PdfExportMode.SplitByChapter))
        vm.selectChapters(setOf(2))
        vm.exportTo("content://tree/destination")
        runCurrent()
        vm.exportTo("content://tree/destination")
        assertFalse(vm.prepareExport(listOf(downloadTask(2)), PdfExportMode.Merge))
    ...
    @Test fun `picker cancellation performs no IO and export errors permit retry`() = runTest(scheduler) {
        ...
        vm.selectChapters(setOf(1))
        vm.prepareExport(listOf(downloadTask()), PdfExportMode.Merge)
        vm.exportTo(null)
        advanceUntilIdle()
        ...
        assertFalse(vm.state.value.exporting)
        ops.exportAction = {}
        assertTrue(vm.prepareExport(listOf(downloadTask()), PdfExportMode.Merge))
        vm.exportTo("content://tree/destination")
        advanceUntilIdle()
    ...
    @Test fun `late file inspection cannot overwrite a newer chapter summary`() = runTest(scheduler) {
        ...
        val vm = DownloadExportViewModel(ops, ToastManager())
        vm.inspect(listOf(downloadTask(1)), "/old")
        runCurrent()
        vm.inspect(listOf(downloadTask(2)), "/new")
        runCurrent()
